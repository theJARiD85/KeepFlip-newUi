...
$backupRoot = Join-Path $projectRoot ".backups\brighter-result-ribbon-$timestamp"

Set-Location $projectRoot

if (-not (Test-Path $targetPath)) {
    throw "Could not find components\scanner\item-analysis-result-stage.tsx."
}
...
Write-Host "Strengthened the lower result typography." -ForegroundColor Green
Write-Host "Brightened labels, body text, values, confidence captions, and footnotes." -ForegroundColor Green
Write-Host "Added restrained glow to accent labels and the median valuation." -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Warning "The typography update was saved, but TypeScript still reports errors. Copy the new error list so only those remaining issues are addressed."
} else {
