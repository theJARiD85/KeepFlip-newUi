...
$targetPath = Join-Path $projectRoot "components\scanner\item-analysis-result-stage.tsx"
$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupRoot = Join-Path $projectRoot ".backups\refined-result-stage-$timestamp"

Set-Location $projectRoot

if (-not (Test-Path $targetPath)) {
    throw "Could not find components\scanner\item-analysis-result-stage.tsx. Run the model-focused layout patch first."
}

New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null
Copy-Item $targetPath (Join-Path $backupRoot "item-analysis-result-stage.tsx") -Force
...
Write-Host "Moved the close button to the upper-left, away from the menu." -ForegroundColor Green
Write-Host "Replaced the bottom bubble cards with an open horizontal data ribbon." -ForegroundColor Green
Write-Host "Reduced the bottom overlay height and transparency so the animation stays visible." -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Warning "The refined layout was saved, but TypeScript still reports errors. Copy the new error list so only those remaining errors are addressed."
} else {
