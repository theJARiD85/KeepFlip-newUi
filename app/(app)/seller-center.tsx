...
export default function SellerCenterRoute() {
  const { user } = useKeepFlipAuth();
  return (
    <>
      <Stack.Screen options={{ title: 'Seller Center', headerShown: true }} />
      {user ? (
        <SellerCenter key={user.$id} ownerId={user.$id} />
      ) : (
        <View style={styles.content}>
          <Text style={styles.text}>Sign in to open Seller Center.</Text>
        </View>
      )}
    </>
  );
...
function SellerCenter({ ownerId }: { ownerId: string }) {
  ...
  return (
    ...
    >
      ...
      {loading ? <ActivityIndicator accessibilityLabel="Loading Seller Center" /> : null}
      ...
      <Button
        title={loading ? 'Refreshing…' : 'Refresh Seller Center'}
