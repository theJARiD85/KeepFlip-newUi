...
const DETECTION_IMAGE_MAX_EDGE = 768;
...
export async function detectObjectsInCapturedPhoto(
  ...
  try {
    const [rawDetections, subjectContour] = await Promise.all([
      ensureDetectorLoaded()
        .then(() => capturedObjectDetector.detectObjects(prepared.uri))
        .catch((error) => {
          if (__DEV__) {
            console.warn(
              "[KeepFlip captured vision] Object labels unavailable; continuing with subject contour.",
              error,
            );
          }
          return [];
        }),
      KeepFlipLocalVisionModule.traceImage(prepared.uri).catch((error) => {
        if (__DEV__) {
          console.warn(
            "[KeepFlip captured vision] Subject contour unavailable; using detected region.",
            error,
          );
        }
        return null;
    ...
    return {
      contours:
        subjectContour != null && subjectContour.points.length >= 6
          ? [{ points: subjectContour.points }]
          : [],
      detections,
      frameHeight,
      frameWidth,
