...
function appwriteModelLocation(modelUrl: string): AppwriteModelLocation {
  let parsedUrl: URL;

  try {
    parsedUrl = new URL(modelUrl);
  } catch {
    throw new Error("The generated model URL is not valid.");
  }
  ...
  if (!match) {
    throw new Error(
      "The generated model URL is not an Appwrite Storage file URL.",
    );
  }
...
export function MeshViewer({
  ...
  useEffect(() => {
    ...
    const downloadModel = async () => {
      try {
        const location = appwriteModelLocation(modelUrl);
        const modelBuffer = await storage.getFileDownload({
