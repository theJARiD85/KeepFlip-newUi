...
export function ListingReadinessPanel({ input, review, onReviewChange, onAspectsChange, onMeasurementsChange, onReadyChange, disabled = false }: {
  ...
  return <View style={styles.panel}>
    <Text style={styles.title}>Listing readiness {fresh && result ? `· ${result.data.score}/100` : ''}</Text>
    <Text style={styles.hint}>Confirm the physical item, then check category requirements, photos, stock and saved seller policies before publishing.</Text>
    ...
    <Text style={styles.hint}>Measurements</Text>
    ...
    {result?.data.requiredAspects.map((aspect) => <View key={aspect.name} style={{ gap: 6 }}><Text style={styles.hint}>Required: {aspect.name}</Text><TextInput accessibilityLabel={`Required item specific ${aspect.name}`} editable={!disabled && !busy} style={styles.input} value={(input.aspects?.[aspect.name] ?? aspect.values).join(' | ')} onChangeText={(value) => onAspectsChange({ ...input.aspects, [aspect.name]: aspect.cardinality === 'MULTI' ? value.split('|').map(part => part.trim()).filter(Boolean) : value ? [value] : [] })} />
      {aspect.cardinality === 'MULTI' ? <Text style={styles.hint}>Separate multiple values with |.</Text> : null}
      {aspect.mode === 'SELECTION_ONLY' ? <Text selectable style={styles.hint}>Use an exact value: {aspect.allowedValues.slice(0, 40).join(', ')}{aspect.allowedValues.length > 40 ? '…' : ''}</Text> : null}
    </View>)}
    {result && !fresh ? <Text style={styles.hint}>The draft changed. Check readiness again.</Text> : null}
...
const styles = StyleSheet.create({ panel: { gap: 12, backgroundColor: theme.colors.surfaceSoft, padding: 16, borderRadius: 18 }, title: { color: theme.colors.text, fontSize: 20 }, hint: { color: theme.colors.textMuted, fontSize: 13, lineHeight: 20 }, row: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 12 }, button: { padding: 12, borderRadius: 10, minHeight: 44 }, selected: { borderWidth: 1, borderColor: theme.colors.gold }, input: { color: theme.colors.text, borderWidth: 1, borderColor: theme.colors.goldMuted, borderRadius: 10, padding: 10, minHeight: 44 }, error: { color: theme.colors.danger, fontSize: 13, lineHeight: 20 }, link: { color: theme.colors.scannerCyan, fontSize: 14 } });
