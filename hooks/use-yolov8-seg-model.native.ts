...
// Keep the segmentation model separate from the existing COCO detector. The
...
export function useYoloV8SegmentationModel(
  ...
  useEffect(() => {
    ...
    async function loadModel() {
      ...
      try {
        ...
        if (!localUri?.startsWith("file://")) {
          throw new Error(`Segmentation model did not resolve to a local file: ${localUri}`);
        }
    ...
    // Delegates are compared by their contents rather than array identity.
